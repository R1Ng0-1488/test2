...
class ListViewTest(TestCase):
	...
	def test_uses_list_template(self):
		'''тест: используется щаблон списка'''
		list_ = List.objects.create()
		response = self.client.get(f'/lists/{list_.id}/')
		self.assertTemplateUsed(response, 'list.html')

	def test_displays_only_items_for_that_list(self):
		'''тест:отображаются только элементы для этого списка'''
		correct_list = List.objects.create()
		Item.objects.create(text='itemey 1', list=correct_list)
		Item.objects.create(text='itemey 2', list=correct_list)
		other_list = List.objects.create()
		Item.objects.create(text='another element 1 of the list', list=other_list)
		Item.objects.create(text='another element 2 of the list', list=other_list)
		...
		self.assertNotContains(response, 'another element 1 of the list')
		self.assertNotContains(response, 'another element 1 of the list')

	def text_passes_correct_list_to_template(self):
		'''тест: передает правильный контекст списка'''
		other_list = List.objects.create()
		correct_list = List.objects.create()
		response = self.client.get(f'/lists/{correct_list.id}/')
	...
	def test_can_save_a_POST_request_to_an_existing_list(self):
		'''тест: можно сохранять post-запрос в существующтй список'''
		other_list = List.objects.create()
		correct_list = List.objects.create()
	...
	def test_for_invalid_input_nothing_saved_to_db(self):
		'''тест на недопустивый ввод: ничего не сохраняется в бд'''
	...
	def test_displays_item_form(self):
		'''тест отображает формы для элемента'''
		list_ = List.objects.create()
		response = self.client.get(f'/lists/{list_.id}/')
		self.assertIsInstance(response.context['form'], ExistingListItemForm)
	...
	def test_duplicate_item_validation_errors_end_up_on_lists_page(self):
		'''тест: ошибки валидации повторябщегося элемента оканчиваются
		на странице списков'''
		...
		expected_error = escape(DUPLICATION_ITEM_ERROR)
...
class NewListViewUnitTest(unittest.TestCase):
	...
	def test_renders_home_template_with_form_if_form_invalid(self,
		...
		'''тест: отображает домашний шаблон с формой, 
		если форма недопустима'''
		mock_form = mockNewListForm.return_value
...
class ShareListTest(TestCase):
	...
	def test_post_redirects_to_lists_page(self):
		'''тест: POST перенаправляет на страницу списков'''
		other_list = List.objects.create()
		correct_list = List.objects.create()
		user = User.objects.create(email='t1m.sadist@gmail.com')

		response = self.client.post(
			f'/lists/{correct_list.id}/share',
			data={'sharee': 't1m.sadist@gmail.com'}
		)
		self.assertRedirects(response, f'/lists/{correct_list.id}/')

	def test_user_can_share_a_list(self):
		'''тест: пользователь может поделиться спаском'''
		list_ = List.objects.create()
		user = User.objects.create(email='a@b.com')
		response = self.client.post(
